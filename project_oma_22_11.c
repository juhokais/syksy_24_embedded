...
void buttonFxn(PIN_Handle handle, PIN_Id pinId){
    ...
    if (program_state == WAITING || program_state == DATA_READY){ //TÄSSÄ UUTTA!!! Napista mittaus pois, joten miksei jotain muutakin...
        buzzTaskFxn(0, 0);
        sprintf(move_msg, " \r\n\0");
        program_state = SENDING;
        /*TAI:
...
void powerOff(void){
    if (main_mode == POWER_OFF){
        ...
        System_printf("Sammuu...pr:%d...main%d\n", program_state, main_mode);//Kommentoida pois??
        System_flush();
