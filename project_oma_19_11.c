/*  Tämä kooditiedosto sisältää kolme tehtävää eli taskia:
    ...
    2. Summeri- eli buzzeritehtävä, jonka toteuttaa funktio buzzTaskFxn(UArg arg0, UArg arg1). Saa argumentteina kaksi etumerkitöntä lukua, jotka ovat joko
    ...
    1. Ensimmäinen painonappikeskytys, jonka käsittelee funktio buttonFxn(). Keskeytyksen lähde on painonapin painallus, jonka laskevalla reunalla keskeytys
       laukaistaan. Käsittelijäfunktio muuttaa tilakoneen tilaa välillä 'mittaustila päällä' - 'mittaustila pois päältä'.
    ...
    tiedostossa UART-liikenne on määritelty tekstimuotoiseksi, eli viestit käsitellään merkkijonoina.
*/
...
char move_msg[100] = ".\r\n \r\n-\r\n \r\n \r\n \r\n"; // ="et" ...default message for testing...
...
void powerOff(void){
    if (main_mode == POWER_OFF){
        buzzTaskFxn(1, 1); //KUULUUKO TÄMÄ?? ENtÄ SITTEN JOS PRINTTAUS POIS...
        System_printf("Sammuu...pr:%d...main%d\n", program_state, main_mode);
        System_flush();
...
Void uartTaskFxn(UArg arg0, UArg arg1) {
    ...
    while(1){
       ...
       if (program_state == RECEIVING){
           ...
           //System_printf("%s##\n", uartTesting);
           //System_flush();
           UART_write(uart, uartTesting, strlen(uartTesting)+1);
