...
void interpret_mpu(void){
    ...
    if (program_state == DATA_READY){
        ...
        if (fabs(*az_po+1.0) > 0.02){ //(1.0+*az_po) > 0.02 ?? kun nostetaan ylös...
            *az_sum_po += fabs(*az_po + 1.0);//tästä + 1.0 pois?? TAI: *az_sum_po += (*1.0 + *az_po)??
        }
        if (((fabs(*ax_sum_po) + fabs(*ay_sum_po)) > 0.06) && (fabs(*az_sum_po) <= 0.02) && (fabs(*ax_prev_po - *ax_po) > 0.01) && (fabs(*ay_prev_po - *ay_po) > 0.01)){
         ...
         }else if (*az_sum_po > 0.50){ //Oli: (fabs(*az_sum_po) > 0.02){ TARVIKO NOSTAA/LASKEA KYNNYSARVOA??
            sprintf(move_msg, "-\r\n\0");
            System_printf("JUMP\n");
            System_flush();
            buzzTaskFxn(0, 0);
