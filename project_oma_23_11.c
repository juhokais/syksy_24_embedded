/*  Tämä kooditiedosto sisältää kolme tehtävää eli taskia:
    ...
    I2c-viestintää käyttää Sensortagin RTOS kommunikaatiossa liikesensori MPU9250:n kanssa. I2c-väylän kautta luetaan data sensorin rekistereistä. Data
    siirretään binäärimuotoisena bitti kerrallaan peräkkäin ajan funktiona. Tiedonsiirtoprotokolla ja tiedonsiirtonopeus määrittelevät mm. yhden bitin
    keston ajallisesti eli tiedon siitä missä kohtaa signaalia bitti alkaa ja päättyy.
...
void buttonFxn_pwr(PIN_Handle handle_pwr, PIN_Id pinId_pwr); // TARVIIKO NIMI MUUTTAA??
...
enum main_state {MEASURE_ON=0, MEASURE_OFF, POWER_OFF}; //POWER_OFF pois??
...
void interpret_mpu(void){
    ...
    if (program_state == DATA_READY){
        ...
        if (fabs(*az_po+1.0) > 0.02){ //if (1.0+*az_po) > 0.02 ?? kun nostetaan ylös...eli az > -1.0  FALL: if(1.0+*az_po) < -0.02...???
            *az_sum_po += fabs(*az_po + 1.0);//tästä + 1.0 pois?? TAI: *az_sum_po += (1.0 + *az_po)??  FALL: *az_sum_po += (1.0+*az_po) --> else if(*az_sum_po < -0.3) --> 'FALL'
        }
        if (((fabs(*ax_sum_po) + fabs(*ay_sum_po)) > 0.06) && (fabs(*az_sum_po) <= 0.02) && (fabs(*ax_prev_po - *ax_po) > 0.01) && (fabs(*ay_prev_po - *ay_po) > 0.01)){
            sprintf(move_msg, ".\r\n\0");
            System_printf("SLIDE\n");
            System_flush();
            buzzTaskFxn(0, 0);
         ...
         }else if (*az_sum_po > 0.50){ //Oli: (fabs(*az_sum_po) > 0.02){ TARVIKO NOSTAA/LASKEA KYNNYSARVOA?? Onko nyt liian korkea jos az_sum lasketaan kuten edellä ehdotettu
            sprintf(move_msg, "-\r\n\0");
            System_printf("JUMP\n");
            System_flush();
            buzzTaskFxn(0, 0);
...
void buttonFxn_pwr(PIN_Handle handle_pwr, PIN_Id pinId_pwr){ //TARVIIKO NIMI MUUTTAA??
    if (program_state == WAITING || program_state == DATA_READY){
        buzzTaskFxn(0, 0); // EI VÄLTTÄMÄTÖN
        sprintf(move_msg, " \r\n\0");
        program_state = SENDING;
        /*TAI:
        uartPrint(uart, " \r\n\0"); //EHKÄ LIIAN RASKAS KESKEYTYKSEEN
        uartPrint(uart, " \r\n\0");
        */ // ='Viestin' loppu???
    }
    /*if (main_mode < 2){
...
/*void powerOff(void){
    if (main_mode == POWER_OFF){
        buzzTaskFxn(1, 1); //KUULUUKO TÄMÄ PIIPPAUS?? ENTÄ SITTEN JOS seuraava PRINTTAUS POIS...
        System_printf("Sammuu...pr:%d...main%d\n", program_state, main_mode);//Kommentoidaan pois lopullisesta (?)
        System_flush();
...
Void uartTaskFxn(UArg arg0, UArg arg1) {
    ...
    while(1){
       ...
       if (program_state == RECEIVING){
           ...
           //UART_write(uart, move_msg, strlen(move_msg)+1);
